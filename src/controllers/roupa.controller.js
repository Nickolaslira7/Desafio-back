...
export const addRoupa = (req, res) => {
    ...
    if (validarRoupa) {
        ...
        contador++
        res.send({message:`Roupa foi adicionada com sucesso ID: ${roupa.id}`,contador});
    }
};
...
export const validarRoupa = (req, res) => {
    ...
    if (name.length < 6 || name.length > 40) {
        ...
        return false
    } if (!imagem.match(/\.(jpeg|jpg|gif|png)$/)) {

        error.push({ error: "url inválida" });
    }
    else {
        return true
    }
